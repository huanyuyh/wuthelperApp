...
class CustomNaviItemView @JvmOverloads constructor(
    context: Context,
    ...
    init {
        ...
        attrs?.let {
            val typedArray = context.obtainStyledAttributes(it, R.styleable.CustomNaviItemView, 0, 0)
            val text = typedArray.getString(R.styleable.CustomNaviItemView_NaviItemText)
            val imageResId = typedArray.getResourceId(R.styleable.CustomNaviItemView_NaviItemImageSrc, -1)
            isSelectedCustom = typedArray.getBoolean(R.styleable.CustomNaviItemView_NaviItemSelected, false)
            setText(text)
            if (imageResId != -1) {
